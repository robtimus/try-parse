...
public class TryParseIntTest {
    ...
    public void testTryParseInt() {
        ...
        for (long i = Integer.MIN_VALUE - 1L; i >= Integer.MIN_VALUE - 5L; i--) {
            checkNegativeOverflow(i);
        }
        for (long i = Integer.MAX_VALUE + 1L; i <= Integer.MAX_VALUE + 5L; i++) {
            checkPositiveOverflow(i);
        }
        for (long i = Integer.MIN_VALUE * 10L, m = 0; m < 5; i *= 10, m++) {
            checkNegativeOverflow(i);
        }
        for (long i = Integer.MAX_VALUE * 10L, m = 0; m < 5; i *= 10, m++) {
            checkPositiveOverflow(i);
        }
    ...
    private void checkNegativeNumber(int i) {
        for (int radix = Character.MIN_RADIX; radix <= Character.MAX_RADIX; radix++) {
            ...
            checkWithAndWithoutSubsequences(i, Integer.toString(i, radix).replace("-", "-0000"), radix);
        ...
        checkWithAndWithoutSubsequences(i, Integer.toString(i).replace("-", "-0000"));
    ...
    private void checkNegativeOverflow(long i) {
        for (int radix = Character.MIN_RADIX; radix <= Character.MAX_RADIX; radix++) {
    ...
    private void check(int expected, String input) {
        ...
        assertEquals(OptionalInt.of(expected), tryParseInt(input));
    }

    private void check(int expected, String input, int radix) {
        Integer.parseInt(input, radix);
        assertEquals(OptionalInt.of(expected), tryParseInt(input, radix));
    }
    ...
    private void check(int expected, String input, int start, int end) {
        ...
        assertEquals(OptionalInt.of(expected), tryParseInt(input, start, end));
    }

    private void check(int expected, String input, int start, int end, int radix) {
        // Integer.parseInt with indexing is not yet available, use substring
        Integer.parseInt(input.substring(start, end), radix);
        assertEquals(OptionalInt.of(expected), tryParseInt(input, start, end, radix));
    }
    ...
    private void checkNull(int radix) {
        assertEquals(OptionalInt.empty(), tryParseInt(null, radix));
    }

    private void checkNull(int start, int end, int radix) {
        assertEquals(OptionalInt.empty(), tryParseInt(null, start, end, radix));
    }
}
